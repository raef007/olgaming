...
class ExchangeController extends BaseController {
    ...
    public function searchWithdraw(){
        ...
        foreach($all_sites as $site) {
            $query  = DB::table('WITHDRAW')
                ->select('*', 'WITHDRAW.reg_date', 'WITHDRAW.money')
                ->where('WITHDRAW.site_id', $site->site_id)
                ->join('MEMBER', 'member.user_id', '=', 'WITHDRAW.user_id');
                
            if ('' != $post_data['from']) {
                $query->where('WITHDRAW.reg_date', '>=', $post_data['from']);
            }
                
            if ('' != $post_data['to']) {
                $query->where('WITHDRAW.reg_date', '<=', $post_data['to']);
            }
    ...
    public function cancelWithdraw($step_list)
    {
        ...
        foreach ($post_data as $site) {
            ...
            if (isset($site[$step_list])) {
                foreach ($site[$step_list] as $request) {
                    if (isset($request['withdraw_check'])) {
                        if ('1' == $request['withdraw_check']) {
                            $withdraw_db->updateStatus($request['w_seq'], $sts);
                        }
                    }
                }
            }
