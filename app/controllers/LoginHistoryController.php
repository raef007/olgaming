...
class LoginHistoryController extends BaseController {
    ...
    public function showGetHistory()
	{
        $all_ok_count   = 0;
        $all_ng_count   = 0;
        $limit          = 3;
        ...
        foreach($all_sites as $site) {
            $site->ok_logs  = DB::table('SIGNIN_LOG')
            ...
            $site->ok_offset    = 0;
            $site->ok_limit     = $limit;
            $site->ok_max       = floor($site->url_count / $site->ok_limit);
            ...
            $site->ng_logs  = DB::table('SIGNIN_LOG')
                ->select('*', 'SIGNIN_LOG.reg_date')
                ->where('SIGNIN_LOG.site_id', $site->site_id)
            ...
            $site->ng_offset    = 0;
            $site->ng_limit     = $limit;
            $site->ng_max       = floor($site->url_count / $site->ng_limit);
